...
#define INTERNAL_TIMEOUT portMAX_DELAY // timeout: wait forever

#define DEST_ADDR "192.168.0.102" //IPv4 destination adress
#define DEST_PORT 5002 // destination port
...
void esp_mesh_external_rx(void *arg){
    ...
    while(is_running){
        ...
        /* fill the array with the data from the matching table*/
        for(int i=0; i<MATCHING_TABLE_SIZE; i++){
            if(matching_table[i]==NULL){//end of the matching table
                break;
            }
            listenSock=matching_table[i]->sock;
            FD_SET(listenSock,&readSet);//add socket descriptor listenSock in the set
        ...
        /* look for the socket that received data */
        for(int i=0; i<MATCHING_TABLE_SIZE; i++){
            ...
            if(FD_ISSET(sock, &readSet)){
                ...
                if(recv_value == 0){
                    ...
                    /*  sends the data to the right node */
...
/**
 * @brief receives data from an internal device for an external device
 * !! only for the root !! 
 * @param arg 
 */
void esp_mesh_external_tx(void *arg){
    esp_err_t mesh_err;
    int err;
...
/**
 * @brief create tasks for internal communication
 * 
 */
...
/**
 * @brief create tasks for external communication
 * 
 */
